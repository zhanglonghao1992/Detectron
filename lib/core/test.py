...
def im_detect_bbox(model, im, target_scale, target_max_size, boxes=None):
    ...
    for k, v in inputs.items():
        workspace.FeedBlob(core.ScopedName(k), v)     # 将一张图的data和im_imfo feed进blob
    workspace.RunNet(model.net.Proto().name)          # run net

    # Read out blobs
    if cfg.MODEL.FASTER_RCNN:
        rois = workspace.FetchBlob(core.ScopedName('rois'))    # 取出rois
        # unscale back to raw image space
        boxes = rois[:, 1:5] / im_scale                       # 将rois缩放到原图的尺寸（原图的最短边被缩放为800）
    ...
    if cfg.TEST.BBOX_REG:
        ...
        pred_boxes = box_utils.clip_tiled_boxes(pred_boxes, im.shape)    # 对边界上的box进行裁剪，使它们不超出图像边界
        if cfg.MODEL.CLS_AGNOSTIC_BBOX_REG:
...
def im_detect_mask(model, im_scale, boxes):
    ...
    for k, v in inputs.items():
        workspace.FeedBlob(core.ScopedName(k), v)             # mask_roi加入blob
    workspace.RunNet(model.mask_net.Proto().name)             # 运行mask_net

    # Fetch masks
    pred_masks = workspace.FetchBlob(
        core.ScopedName('mask_fcn_probs')       # 'mask_fcn_probs'（仅在test阶段有该blob）：将预测出的mask通过一个sigmoid函数，使得值都在0-1之间
    ).squeeze()
...
def segm_results(cls_boxes, masks, ref_boxes, im_h, im_w):
    ...
    for j in range(1, num_classes):
        ...
        for _ in range(cls_boxes[j].shape[0]):
            ...
            mask = cv2.resize(padded_mask, (w, h))           # 将pad过的30*30的mask resize到h*w
            mask = np.array(mask > cfg.MRCNN.THRESH_BINARIZE, dtype=np.uint8)     # 用阈值0.5将mask中的值（前面已经用sigmoid变为0-1之间）变为0或1。mask变为二值mask
