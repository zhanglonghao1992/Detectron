...
def initialize_gpu_from_weights_file(model, weights_file, gpu_id=0):
    ...
    unscoped_param_names = OrderedDict()  # Print these out in model order     orderdict是一个按输入顺序储存的字典
    ...
    with c2_utils.NamedCudaScope(gpu_id):
        for unscoped_param_name in unscoped_param_names.keys():
            ...
            workspace.FeedBlob(                                 # 把src_blobs[src_name]中的内容添加进workspace中名为dst_name的blob中
                dst_name,
