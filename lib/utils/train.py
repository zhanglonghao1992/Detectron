...
def train_model():
    ...
    model, weights_file, start_iter, checkpoints, output_dir = create_model()    # 构造model，计算图
    if 'final' in checkpoints:
        # The final model was found in the output directory, so nothing to do
        return checkpoints

    setup_model_for_training(model, weights_file, output_dir)     # 读数据，创建roidb
    ...
    for cur_iter in range(start_iter, cfg.SOLVER.MAX_ITER):
        ...
        lr = model.UpdateWorkspaceLr(cur_iter, lr_policy.get_lr_at_iter(cur_iter))       # 注意：存在warm up阶段（默认前500个iters）
        ...
        if (cur_iter + 1) % CHECKPOINT_PERIOD == 0 and cur_iter > start_iter:
            ...
            nu.save_model_to_weights_file(checkpoints[cur_iter], model)        # 将当前iter的参数存入checkpoints[cur_iter]
    ...
    nu.save_model_to_weights_file(checkpoints['final'], model)          # 将训练结束后的model的参数存入checkpoints['final']
...
def setup_model_for_training(model, weights_file, output_dir):
    ...
    if weights_file:      # 如 https://s3-us-west-2.amazonaws.com/detectron/ImageNetPretrained/MSRA/R-50.pkl
        # Override random weight initialization with weights from a saved model
        nu.initialize_gpu_from_weights_file(model, weights_file, gpu_id=0)    # 使用预训练模型初始化网络参数
    # Even if we're randomly initializing we still need to synchronize
    # parameters across GPUs
    nu.broadcast_parameters(model)           # 同步各gpu中的parameters
    workspace.CreateNet(model.net)

    logger.info('Outputs saved to: {:s}'.format(os.path.abspath(output_dir)))
    dump_proto_files(model, output_dir)    # 把training network 和 parameter initialization network保存成prototxt
...
def add_model_training_inputs(model):
    ...
    roidb = combined_roidb_for_training(                                 # 创建roidb
