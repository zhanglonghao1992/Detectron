...
def generalized_rcnn(model):
    ...
    return build_generic_detection_model(
        model,
        get_func(cfg.MODEL.CONV_BODY),      # FPN.add_fpn_ResNet50_conv5_body
        add_roi_box_head_func=get_func(cfg.FAST_RCNN.ROI_BOX_HEAD),   # fast_rcnn_heads.add_roi_2mlp_head
        add_roi_mask_head_func=get_func(cfg.MRCNN.ROI_MASK_HEAD),     # mask_rcnn_heads.mask_rcnn_fcn_head_v1up4convs
...
def create(model_type_func, train=False, gpu_id=0):
    ...
    model = DetectionModelHelper(
        name=model_type_func,   # generalized_rcnn
        train=train,
        num_classes=cfg.MODEL.NUM_CLASSES,
        init_params=train      # true
    )
...
def get_func(func_name):
    ...
    try:
        ...
        return getattr(module, parts[-1])     # 返回module对象中的parts[-1]方法，打印出来
    except Exception:
...
def build_generic_detection_model(
    ...
    def _single_gpu_build_func(model):
        ...
        blob_conv, dim_conv, spatial_scale_conv = add_conv_body_func(model)      # 调用FPN中的add_fpn_ResNet50_conv5_body（model）函数
        # 返回值：
        # blob_conv = ['fpn_res5_2_sum_subsampled_2x','fpn_res5_2_sum','fpn_res4_5_sum','fpn_res3_3_sum','fpn_res2_2_sum']
        # dim_conv = 256
        # spatial_scales_conv = [1/64,1/32,1/16,1/8,1/4]
        ...
        if cfg.FPN.FPN_ON:
            ...
            # 去掉了blob_conv中的'fpn_res5_2_sum_subsampled_2x'，它只在rpn中用到
            # blob_conv=['fpn_res5_2_sum','fpn_res4_5_sum','fpn_res3_3_sum','fpn_res2_2_sum']
            # spatial_scale_conv=[1/32,1/16,1/8,1/4]
        ...
        if not cfg.MODEL.RPN_ONLY:
            ...
            head_loss_gradients['box'] = _add_fast_rcnn_head(
                model, add_roi_box_head_func, blob_conv, dim_conv,        # add_roi_box_head_func为fast_rcnn_heads.add_roi_2mlp_head
                spatial_scale_conv
            )
        ...
        if cfg.MODEL.MASK_ON:
            ...
            head_loss_gradients['mask'] = _add_roi_mask_head(
                model, add_roi_mask_head_func, blob_conv, dim_conv,      # add_roi_mask_head_func为mask_rcnn_heads.mask_rcnn_fcn_head_v1up4convs
                spatial_scale_conv
            )
...
def _add_fast_rcnn_head(
    ...
    blob_frcn, dim_frcn = add_roi_box_head_func(        # blob_frcn为fc7，dim_frcn为1024
