...
def add_fpn_onto_conv_body(
    ...
    conv_body_func(model)    # ResNet.add_ResNet50_conv5_body（model）函数
    blobs_fpn, dim_fpn, spatial_scales_fpn = add_fpn(
        model, fpn_level_info_func()
    )
    # blobs_fpn=[fpn_res5_2_sum_subsampled_2x,fpn_res5_2_sum,fpn_res4_5_sum,fpn_res3_3_sum,fpn_res2_2_sum]
    # dim_fpn=256
    # spatial_scales_fpn=[1/64,1/32,1/16,1/8,1/4]
...
def add_fpn(model, fpn_level_info):
    ...
    for i in range(num_backbone_stages):
        ...
        blobs_fpn += [fpn_blob]      # blobs_fpn=[fpn_res5_2_sum,fpn_res4_5_sum,fpn_res3_3_sum,fpn_res2_2_sum]
    ...
    if not cfg.FPN.EXTRA_CONV_LEVELS and max_level == HIGHEST_BACKBONE_LVL + 1:
        ...
        blobs_fpn.insert(0, P6_blob)   # blobs_fpn=[fpn_res5_2_sum_subsampled_2x,fpn_res5_2_sum,fpn_res4_5_sum,fpn_res3_3_sum,fpn_res2_2_sum]
...
def get_min_max_levels():
    ...
    if cfg.FPN.MULTILEVEL_RPN and cfg.FPN.MULTILEVEL_ROIS:
        max_level = max(cfg.FPN.RPN_MAX_LEVEL, cfg.FPN.ROI_MAX_LEVEL)     # 6
        min_level = min(cfg.FPN.RPN_MIN_LEVEL, cfg.FPN.ROI_MIN_LEVEL)     # 2
...
def add_fpn_rpn_outputs(model, blobs_in, dim_in, spatial_scales):
    ...
    num_anchors = len(cfg.FPN.RPN_ASPECT_RATIOS)     # 3
    dim_out = dim_in     # 256
